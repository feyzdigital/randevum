...
function generateSalonQRCardUrl(salon) {
    const salonUrl = 'https://randevum.feyz.digital/' + (salon.category || 'berber') + '/' + salon.slug + '/';
...
function checkAuth() {
    const session = localStorage.getItem('randevum_admin');
    if (session) {
        ...
        else { localStorage.removeItem('randevum_admin'); renderLogin(); }
    } else renderLogin();
}
...
function login() {
    ...
    if (pin === ADMIN_CONFIG.superAdminPin) {
        ...
        localStorage.setItem('randevum_admin', JSON.stringify({ pin, expiry: expiry.toISOString() }));
        AdminState.isLoggedIn = true; loadAllData();
    } else { showToast('Gecersiz PIN!', 'error'); document.getElementById('pinInput').value = ''; }
}

function logout() { localStorage.removeItem('randevum_admin'); AdminState.isLoggedIn = false; renderLogin(); }
...
function renderDetail() {
    ...
    let h = '<div class="view-header"><div><button onclick="nav(\'salons\')" class="btn btn-outline btn-sm">&larr; Geri</button><h1>' + c.icon + ' ' + esc(s.name) + '</h1><p>/' + s.slug + ' &middot; <span class="status-badge ' + (s.active ? 'active' : 'inactive') + '">' + (s.active ? 'Aktif' : 'Pasif') + '</span></p></div><div class="header-actions"><a href="/' + (s.category || 'berber') + '/' + s.slug + '/" target="_blank" class="btn btn-outline">Sayfayi Ac</a><a href="/' + (s.category || 'berber') + '/' + s.slug + '/yonetim/?admin=true" target="_blank" class="btn btn-outline">Yonetim Paneli</a><button onclick="showEditModal(\'' + s.id + '\')" class="btn btn-primary">Duzenle</button></div></div>';
    ...
    if (AdminState.detailTab === 'info') {
        ...
        h += '<div class="detail-grid"><div class="card"><h3>&#128274; Giris Bilgileri</h3><div class="info-list"><div class="info-row"><span class="info-label">Telefon</span><span class="info-value"><strong>' + (s.mobilePhone || s.phone || '-') + '</strong></span></div><div class="info-row"><span class="info-label">PIN Kodu</span><span class="info-value"><code style="font-size:1.2rem;background:var(--slate-100);padding:0.25rem 0.75rem;border-radius:4px">' + (s.pin || 'Belirlenmemis') + '</code></span></div><div class="info-row"><span class="info-label">Yonetim Paneli</span><span class="info-value" style="word-break:break-all;font-size:0.85rem">' + location.origin + '/' + (s.category || 'berber') + '/' + s.slug + '/yonetim/?admin=true</span></div></div><div style="display:flex;flex-direction:column;gap:0.5rem;margin-top:1rem"><a href="/' + (s.category || 'berber') + '/' + s.slug + '/yonetim/?admin=true" target="_blank" class="btn btn-primary">🚀 Yonetim Paneline Git (Admin)</a><button onclick="showChangePinModal(\'' + s.id + '\')" class="btn btn-outline">PIN Degistir</button></div></div>';
...
async function generateSalonQRCard(salon) {
    ...
    ctx.fillText('randevum.feyz.digital', 200, 485);
